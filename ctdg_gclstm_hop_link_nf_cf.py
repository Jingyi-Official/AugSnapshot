...
class LinkPredictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        self.lins.append(torch.nn.Linear(in_channels + 100, hidden_channels))
    ...
    def forward(self, x_i, x_j, hop=None):
...
def pad_sequences(node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
    ...
    if isinstance(node_interact_times, torch.Tensor):
...
def test_tgb(
    ...
    node_feat = backbone.node_raw_features
    ...
    for batch in test_loader:
        ...
        for idx, neg_batch in enumerate(neg_batch_list):
            ...
            #             np.concatenate(
            #                 ([np.array([pos_dst[idx]]), np.array(neg_batch)]),
            #                 axis=0,
            #             ),
            #             device=args.device,
            #         )
            query_dst = torch.cat((pos_dst[idx].unsqueeze(0), torch.tensor(neg_batch,device=args.device)), dim=0)
            ...
            # ADD
                ...
                src_neighbors, src_neighbors_edge, src_neighbors_times = neighbor_sampler.get_all_first_hop_neighbors(node_ids=query_src, node_interact_times=query_t)
                dst_neighbors, dst_neighbors_edge, dst_neighbors_times = neighbor_sampler.get_all_first_hop_neighbors(node_ids=query_dst, node_interact_times=query_t)

                src_pad_neighbors, src_pad_neighbors_edge, src_pad_neighbors_times = \
                    pad_sequences(node_ids=query_src, node_interact_times=query_t, nodes_neighbor_ids_list=src_neighbors,
                ...
                dst_pad_neighbors, dst_pad_neighbors_edge, dst_pad_neighbors_times = \
                    pad_sequences(node_ids=query_dst, node_interact_times=query_t, nodes_neighbor_ids_list=dst_neighbors,
                                    nodes_edge_ids_list=dst_neighbors_edge, nodes_neighbor_times_list=dst_neighbors_times,
                                    patch_size=1, max_input_sequence_length=32)
                src_padded_nodes_neighbor_co_occurrence_features, dst_padded_nodes_neighbor_co_occurrence_features = dynamic_backbone.neighbor_co_occurrence_encoder(src_padded_nodes_neighbor_ids=src_pad_neighbors, dst_padded_nodes_neighbor_ids=dst_pad_neighbors)
                    
                src_padded_nodes_neighbor_node_raw_features, src_padded_nodes_edge_raw_features, src_padded_nodes_neighbor_time_features = \
                        dynamic_backbone.get_features(node_interact_times=query_t.cpu().numpy(), padded_nodes_neighbor_ids=src_pad_neighbors,
                                        padded_nodes_edge_ids=src_pad_neighbors_edge, padded_nodes_neighbor_times=src_pad_neighbors_times, time_encoder=dynamic_backbone.time_encoder)

                dst_padded_nodes_neighbor_node_raw_features, dst_padded_nodes_edge_raw_features, dst_padded_nodes_neighbor_time_features = \
                        dynamic_backbone.get_features(node_interact_times=query_t.cpu().numpy(), padded_nodes_neighbor_ids=dst_pad_neighbors,
                                        padded_nodes_edge_ids=dst_pad_neighbors_edge, padded_nodes_neighbor_times=dst_pad_neighbors_times, time_encoder=dynamic_backbone.time_encoder)
                ...
                src_patches_nodes_neighbor_time_features, src_patches_nodes_neighbor_co_occurrence_features = \
                    dynamic_backbone.get_patches(padded_nodes_neighbor_node_raw_features=src_padded_nodes_neighbor_node_raw_features,
                                    ...
                                    patch_size=dynamic_backbone.patch_size)
                ...
                dst_patches_nodes_neighbor_time_features, dst_patches_nodes_neighbor_co_occurrence_features = \
                    dynamic_backbone.get_patches(padded_nodes_neighbor_node_raw_features=dst_padded_nodes_neighbor_node_raw_features,
                ...
                src_patches_nodes_neighbor_node_raw_features = dynamic_backbone.projection_layer['node'](src_patches_nodes_neighbor_node_raw_features)
                src_patches_nodes_edge_raw_features = dynamic_backbone.projection_layer['edge'](src_patches_nodes_edge_raw_features)
                src_patches_nodes_neighbor_time_features = dynamic_backbone.projection_layer['time'](src_patches_nodes_neighbor_time_features)
                src_patches_nodes_neighbor_co_occurrence_features = dynamic_backbone.projection_layer['neighbor_co_occurrence'](src_patches_nodes_neighbor_co_occurrence_features)

                dst_patches_nodes_neighbor_node_raw_features = dynamic_backbone.projection_layer['node'](dst_patches_nodes_neighbor_node_raw_features)
                dst_patches_nodes_edge_raw_features = dynamic_backbone.projection_layer['edge'](dst_patches_nodes_edge_raw_features)
                dst_patches_nodes_neighbor_time_features = dynamic_backbone.projection_layer['time'](dst_patches_nodes_neighbor_time_features)
                dst_patches_nodes_neighbor_co_occurrence_features = dynamic_backbone.projection_layer['neighbor_co_occurrence'](dst_patches_nodes_neighbor_co_occurrence_features)
                ...
                patches_data = [patches_nodes_neighbor_node_raw_features, patches_nodes_neighbor_co_occurrence_features]
                ...
                # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
                patches_data = patches_data.reshape(batch_size, src_num_patches + dst_num_patches, 2 * dynamic_backbone.channel_embedding_dim)
                

                with torch.no_grad():
                    y_pred = link_pred(h[query_src], h[query_dst],patches_data.mean(dim=1))
                y_pred = y_pred.squeeze(dim=-1).detach()  
...
if __name__ == '__main__':
    ...
    for seed in range(args.seed, args.seed + args.num_runs):
        ...
        node_feat = torch.randn((num_nodes, node_feat_dim)).to(args.device).cpu().numpy()
        edge_feat = torch.randn((len(set(data['train_data']['edge_index'].keys())), node_feat_dim)).to(args.device).cpu().numpy()

        link_pred = LinkPredictor(hidden_dim, hidden_dim, 1,
                                2, 0.2).to(args.device)

        dynamic_backbone = DyGFormer(node_raw_features=node_feat, edge_raw_features=edge_feat, neighbor_sampler=train_neighbor_sampler,
                                         time_feat_dim=100, channel_embedding_dim=50, patch_size=2,
                                         num_layers=2, num_heads=2, dropout=0.1,
                                         max_input_sequence_length=64, device=args.device).to(args.device)
        ...
        for epoch in range(num_epochs):
            ...
            for snapshot_idx in range(train_data['time_length']):
                ...
                if (snapshot_idx == 0): #first snapshot, feed the current snapshot
                    ...
                    h, h_0, c_0 = model(dynamic_backbone.node_raw_features, cur_index, edge_attr, h_0, c_0)
                    ...
                    if ('edge_attr' not in train_data):
                        ...
                        raise NotImplementedError("Edge attributes are not yet supported")
                    h, h_0, c_0 = model(dynamic_backbone.node_raw_features, prev_index, edge_attr, h_0, c_0)
                ...
                # ADD
                    ...
                    src_neighbors, src_neighbors_edge, src_neighbors_times = dynamic_backbone.neighbor_sampler.get_all_first_hop_neighbors(node_ids=pos_index[0], node_interact_times=node_interact_times)
                    dst_neighbors, dst_neighbors_edge, dst_neighbors_times = dynamic_backbone.neighbor_sampler.get_all_first_hop_neighbors(node_ids=pos_index[1], node_interact_times=node_interact_times)
                    ...
                    src_padded_nodes_neighbor_node_raw_features, src_padded_nodes_edge_raw_features, src_padded_nodes_neighbor_time_features = \
                            dynamic_backbone.get_features(node_interact_times=node_interact_times, padded_nodes_neighbor_ids=src_pad_neighbors,
                                            padded_nodes_edge_ids=src_pad_neighbors_edge, padded_nodes_neighbor_times=src_pad_neighbors_times, time_encoder=dynamic_backbone.time_encoder)

                    dst_padded_nodes_neighbor_node_raw_features, dst_padded_nodes_edge_raw_features, dst_padded_nodes_neighbor_time_features = \
                            dynamic_backbone.get_features(node_interact_times=node_interact_times, padded_nodes_neighbor_ids=dst_pad_neighbors,
                                            padded_nodes_edge_ids=dst_pad_neighbors_edge, padded_nodes_neighbor_times=dst_pad_neighbors_times, time_encoder=dynamic_backbone.time_encoder)
                    ...
                    pos_out = link_pred(h[pos_index[0]], h[pos_index[1]],patches_data.mean(dim=1))
                    neg_out = link_pred(h[pos_index[0]], h[neg_dst], patches_data.mean(dim=1))
            ...
            if epoch > 100:
                ...
                val_metrics, h, h_0, c_0 = test_tgb(full_neighbor_sampler, args.hop, h, h_0, c_0, val_loader, val_snapshots, ts_list,
                    dynamic_backbone,model, link_pred,neg_sampler,evaluator,metric, split_mode='val')
                ...
                if (val_metrics > best_val):
                    ...
                    dynamic_backbone,model, link_pred,neg_sampler,evaluator,metric, split_mode='test')
