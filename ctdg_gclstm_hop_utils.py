...
def get_neighbor_sampler_ct(data, sample_neighbor_strategy: str = 'uniform', time_scale: str = 'hourly', time_scaling_factor: float = 0.0, seed: int = None):
    """
    ...
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(data['src'].detach().cpu(), data['dst'].detach().cpu(), data['y'].detach().cpu(), data['t'].detach().cpu()):
        bin_ts = ceiling_division(node_interact_time.item()-start_time, interval_size)
        unix_ts = start_time + bin_ts * interval_size
        unix_ts = torch.tensor(int(unix_ts))
        adj_list[src_node_id].append((dst_node_id, edge_id, unix_ts))
        adj_list[dst_node_id].append((src_node_id, edge_id, unix_ts))
...
def get_neighbor_sampler_dt(data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    """
    get neighbor sampler
    :param data: Data
    :param sample_neighbor_strategy: str, how to sample historical neighbors, 'uniform', 'recent', or 'time_interval_aware''
    ...
    """
    edge_list = []
    edge_index_dict = data['edge_index']  # Dictionary of edge lists per time step
    ts_map_dict = data['ts_map']  # Number of snapshots

    edge_id = 0  # Initialize edge ID counter

    for ts, edge_index in edge_index_dict.items():
        ts_map = ts_map_dict[ts]
        src_nodes = edge_index[0].tolist()  # Convert tensor to list
        dst_nodes = edge_index[1].tolist()  # Convert tensor to list

        for src, dst in zip(src_nodes, dst_nodes):
            edge_list.append((src, dst, edge_id, ts_map))  # Append tuple (src, dst, edge_id, timestamp)
            edge_id += 1  # Increment edge ID
    
    edge_list = np.array(edge_list)
    max_node_id = max(edge_list[:,0].max(), edge_list[:,1].max())
    # the adjacency vector stores edges for each node (source or destination), undirected
    # adj_list, list of list, where each element is a list of triple tuple (node_id, edge_id, timestamp)
    # the list at the first position in adj_list is empty
    adj_list = [[] for _ in range(max_node_id + 1)]
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(edge_list[:,0], edge_list[:,1], edge_list[:,2], edge_list[:,3]):
